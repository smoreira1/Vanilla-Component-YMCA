window.ymcaSearchComponent = {
    ...
    init: async function (filters, loggedIn, async, renderResultsNow, apiPath) {
            this.config(filters, loggedIn, async, renderResultsNow, apiPath);
            //NOTE: FIX ME! THESE ASYNC CALLS DO NOT DEPEND ON EACH OTHER.
            this.programs = await fetch(`${this.apiPath}/programs.json` , {mode:'cors'}).then((response) => {
                return response.json();
            });
            this.familyCenters = await fetch(`${this.apiPath}/familyCenters.json` , {mode:'cors'}).then((response) => {
                return response.json();
            });
            this.render();
        ...
        config: function (filters, loggedIn, async, renderResultsNow, apiPath) {
        ...
        getSearchResultsRender: function () {
            return fetch(`${this.apiPath}/ymcaPrograms.json`)
                .then((response) => {
                    return response.json();
                })
                .then((data) => {
